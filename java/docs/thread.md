...
커널 모드와 유저 모드는 컴퓨터 시스템의 안정성과 보안을 지키기 위해 CPU가 실행되는 권한 수준을 다르게 부여받는다

커널은 하드웨어에 직접 접근할 수 있는 권한(privilege) 모드에서 실행되는 시스템 소프트웨어로 자원 관리, 프로세스 스케줄링, 메모리 관리 등 운영체제의 핵심 역할을 수행한다 (하드웨어와 소프트웨어 사이의 중재)

컴퓨터의 부트 과정(BIOS/UEFI -> 부트로더 -> 커널 -> OS -> 유저 프로세스)에서 커널이 시작되어, 컴퓨터가 꺼질 때까지 동작한다

운영체제는 커널 외에도 시스템 데몬, 윈도우 시스템, 시스템 라이브러리/유틸리티 등으로 구성되어 있는데 이러한 프로그램은 커널 모드가 아닌 유저 모드에서 실행된다

유저 영역에서 실행되는 프로세스는 직접 하드웨어를 제어할 수 없고 오로지 커널을 통해야만 한다

단, 직접 커널에 접근할 수 없으며 커널에서 제공하는 '시스템 콜' 이라고 하는 인터페이스를 통해 하드웨어에 접근할 수 있다
...
**프로세스**는 **컴퓨팅 자원의 단위**이자 운영체제로부터 실행된 프로그램의 인스턴스로 독립적인 메모리 공간을 가진다 (프로그램 - 디스크, 프로세스 - 램)

하나의 프로세스는 컴퓨터 자원(메모리, 파일 핸들링 등)을 소유하며 이러한 자원을 사용하는 한 개 이상의 '커널 스레드'를 포함한다

기본적으로 각 프로세스는 격리되어 서로가 점유하고 있는 자원을 공유하지 않는다 (독립된 가상 메모리 공간 보유)

프로세스 간 공유가 필요하면 명시적으로 공유 메모리, 소켓 등 IPC를 사용해야 한다

**스레드**는 **프로세스 내의 경량 실행 단위**로 하나의 프로세스 내에 여러 개의 스레드가 동시에 실행될 수 있다

프로세스와 달리 각 스레드는 같은 프로세스의 메모리를 공유할 수 있다 -> 효율적인 자원 사용

다만 여러 스레드가 동일 자원을 동시에 사용하게 되면 경쟁 조건, 교착 상태 등의 동시성 문제가 발생한다 -> 이러한 부작용을 방지하거나 해결할 수 있어야 한다

프로세스가 커널 영역과 유저 영역에서 실행되듯이 스레드도 **유저 레벨 스레드**와 **커널 레벨 스레드**로 나뉜다 (관리 주체의 차이)

### 커널 레벨 스레드

커널 레벨 스레드는 **운영체제 커널이 직접 관리하는 기본적인 실행 단위** (CPU 스케줄링 단위)로 커널에서 인식하고 처리하므로 운영체제가 스레드의 스케줄링, 생성 및 관리를 수행할 수 있다

**유저 프로세스 안에 존재하며 커널에 의해 직접 관리되는 스레드일 뿐 '커널 프로세스 내의 스레드'를 의미하지 않는다** (커널 스레드가 아닌 커널 레벨 스레드임을 유의)

커널 레벨 스레드 = OS 스레드

커널이 인지하는 스케줄링 단위이므로 커널 스케줄러에 의해 CPU를 할당받아 유저 코드를 실행한다

운영체제는 프로세스를 생성할 때 최소 1개의 커널 스레드를 생성하는데 이러한 스레드를 **Native Thread** 또는 **Non-Green Thread** 라고 한다

커널의 스케줄링 정책을 따르기 때문에 멀티 코어 CPU에서 병렬로 실행될 수 있다

커널 레벨 스레드의 스택 크기는 보통 수백 KB에서 수 MB에 달한다 

따라서 커널 레벨 스레드가 과하게 많아지면 커널의 작업 부담(커널 스레드 스케줄링, 컨텍스트 스위칭)이 커져 전체 시스템 성능이 저하되거나 스택 메모리 할당을 하다가 시스템 전체 메모리를 고갈시킬 수 있다

### 유저 레벨 스레드

유저 레벨 스레드는 **유저 공간의 라이브러리에서 관리하는 실행 단위**로 커널이 인식하지 못하여 커널에 의해 직접 관리를 받지 않는다

유저 공간에서만 생성되고 관리되는 논리적인 스레드로 커널 레벨 스레드 위에서 동작한다

CPU는 커널이 스케줄링한 실행 단위만 실행할 수 있다
...
유저 레벨 스레드는 커널 레벨 스레드와 달리 일반적으로 수십 바이트에서 수백 바이트 정도만 차지한다

유저 레벨 스레드가 이렇게 작은 메모리 공간을 가질 수 있는 이유는 다음과 같다
- 커널 스택 부재: 커널 스레드는 최소 수십 KB에서 1~2MB에 달하는 고정된 커널 스택 메모리를 할당받는데, 유저 레벨 스레드는 이러한 스택이 필요없거나 애플리케이션의 힙 메모리에 작은 스택을 할당받아 사용한다
- 경량 데이터 구조: 유저 스레드는 주로 스레드 상태, PC, 레지스터 값 등만을 저장한다
- 단일 커널 레벨 스레드 위에서의 멀티 플렉싱: 여러 유저 레벨 스레드가 소수의 커널 레벨 스레드 위에서 작동하기 때문에 많은 수의 유저 스레드가 동시에 존재하더라도 실제로는 단일 커널 레벨 스레드에 의해 실행되므로 메모리 부담이 훨씬 적다

## 스레드 매핑 모델

스레드 매핑 모델은 유저 레벨 스레드와 커널 레벨 스레드를 연결하는 방식으로 유저 레벨의 실행 흐름이 커널 스케줄링 단위와 어떻게 매핑되는지를 정의한다

모델에 따라 프로세스의 성능, 확장성, 블로킹 동작, 스레드 컨텍스트 스위칭 비용이 달라진다
...
유저 공간의 스레드 라이브러리가 스케줄링하므로 스레드 간 컨텍스트 스위칭이 매우 빠르다 (스레드 스케줄링을 하기 위해 커널에 접근하지 않아도 되므로)
...
즉, 유저 레벨 스레드와 커널 레벨 스레드는 서로 다른 두 종류의 객체가 2개 생기는 것이 아니고 실제로 실행되는 건 커널 레벨 스레드이다
- 자바 Thread 객체 (유저 공간)
- OS 커널 스레드 (커널 공간)
...
스레드 간 데이터를 공유하고 선점형으로 동작한다면 일반적인 유저 스레드라고 부른다 (Erlang처럼 스레드끼리 데이터를 공유하지 않는 경우도 있는데 이 경우엔 그냥 프로세스라고 부른다)
...
OS가 해당 프로그램을 실행하기 위한 프로세스를 생성하며 프로세스에게 독립된 메모리(가상 메모리) 주소 공간을 할당하고 PCB를 생성한다

그 다음 메인 함수가 실행될 스레드를 커널 스레드와 TCB를 생성한다

CPU의 스케줄링에 따라 커널 스레드가 CPU를 할당받아 프로그램의 메인 함수가 실행된다

유저 코드에서 스레드를 더 만들면 일반적인 경우 OS가 커널 스레드를 추가적으로 생성한다 (1:1 모델)

또는 유저 스레드를 만들어 런타임에서 자체적으로 관리할 수도 있다 (M:N 모델)
...
동시성 문제에는 **경쟁 상태(Race Condition)**, **교착 상태(Deadlock)**, **기아(Starvation)**, **라이브락(Livelock)**이 있다
