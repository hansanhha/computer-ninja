...
- [프로세스와 스레드](#프로세스와-스레드)
- [스레드 매핑 모델](#스레드-매핑-모델)
- [스케줄링과 컨텍스트 스위칭](#스케줄링과-컨텍스트-스위칭)
- [멀티 코어와 멀티 스레딩 (동시성 문제)](#멀티-코어와-멀티-스레딩-동시성-문제)
- [메인 루틴, 서브 루틴, 코루틴](#메인-루틴-서브-루틴-코루틴)
- [제어권(블로킹과 논블로킹), 작업 순서 흐름(동기와 비동기)](#제어권블로킹과-논블로킹-작업-순서-흐름동기와-비동기)
...
컴퓨터의 메모리는 크게 **커널 영역**과 **유저(런타임) 영역**으로 철저하게 분리되어 있다.

CPU 실행 권한 수준을 커널 모드와 유저 모드로 나눠 컴퓨터 시스템의 안정성과 보안을 유지한다.

커널은 하드웨어에 직접 접근할 수 있는 권한(privilege) 모드에서 실행되는 시스템 소프트웨어로 자원 관리, 프로세스 스케줄링, 메모리 관리 등 운영체제의 핵심 역할을 수행한다. (하드웨어와 소프트웨어 사이의 중재)

컴퓨터의 부트 과정에서 커널이 시작되어 컴퓨터가 꺼질 때까지 동작한다. (BIOS/UEFI -> 부트로더 -> **커널** -> OS -> 유저 프로세스)

운영체제는 커널 외에도 시스템 데몬, 윈도우 시스템, 시스템 라이브러리/유틸리티 등으로 구성되어 있는데 이러한 프로그램은 커널 모드가 아닌 유저 모드에서 실행된다.

유저 영역에서 실행되는 프로세스는 직접 하드웨어를 제어할 수 없고 시스템 콜을 호출하여 커널에게 하드웨어 제어를 요청한다.
...
**프로세스**는 **컴퓨팅 자원의 단위**이자 운영체제로부터 실행된 프로그램의 인스턴스로 각 프로세스는 독립적인 메모리 공간을 가진다. (독립된 가상 메모리 공간 보유)

하나의 프로세스는 컴퓨터 자원(메모리, 파일 핸들링 등)을 소유하며 이러한 자원을 사용하는 한 개 이상의 '커널 스레드'를 포함한다.

프로세스 간 공유가 필요하면 공유 메모리, 소켓 등 IPC를 이용한다.

**스레드**는 **프로세스 내의 경량 실행 단위**로 하나의 프로세스 내에 여러 개의 스레드가 동시에 실행될 수 있다.

프로세스와 달리 각 스레드는 같은 프로세스의 메모리를 공유할 수 있다. -> 효율적인 자원 사용

다만 여러 스레드가 동일 자원을 동시에 사용하게 되면 경쟁 조건, 교착 상태 등의 동시성 문제가 발생한다.

스레드는 관리 주체에 따라 크게 커널 레벨 스레드와 유저 레벨 스레드로 나뉜다.

### 커널 레벨 스레드

커널 레벨 스레드는 커널 메모리 영역 내에 존재하는 스레드다.

리눅스: `task_struct`

윈도우: `ETHREAD/KTHREAD`

프로세스가 처음 실행되면 커널은 이 프로세스를 관리하기 위해 내부적으로 최소 1개의 커널 레벨 스레드(메인 스레드)를 생성한다.

커널은 유저 프로세스 내의 커널 레벨 스레드만 인식할 수 있고, CPU 스케줄링의 대상으로 삼을 수 있다. (커널의 스케줄링 정책을 따르기 때문에 멀티 코어 CPU에서 병렬로 실행될 수 있음)

-> **프로세스 자체가 커널에 의해 스케줄링되는 것이 아니라 프로세스에 속한 커널 레벨 스레드들이 스케줄링되어 CPU 자원을 할당받아 코드를 실행한다.**

하나의 커널 레벨 스레드가 I/O 작업 등으로 인해 블로킹되어도 커널이 다른 커널 스레드를 다른 CPU 코어에 배치하여 프로세스 전체가 멈추는 것을 막을 수 있다.

커널 레벨 스레드가 시스템 콜을 호출하면 커널 영역에 따로 커널 스택이 할당된다. (스택 크기는 보통 수백 KB에서 수 MB에 달함)

따라서 커널 레벨 스레드가 과하게 많아지면 커널의 작업 부담(커널 스레드 스케줄링, 컨텍스트 스위칭)이 커져 전체 시스템 성능이 저하되거나 스택 메모리 할당을 하다가 시스템 전체 메모리를 고갈시킬 수 있다.

생성, 제거, 컨텍스트 스위칭할 때마다 시스템 콜을 통해 커널 모드로 진입해야 하므로 유저 레벨 스레드보다 상대적으로 무겁다.

### 유저 레벨 스레드

유저 레벨 스레드는 **유저 공간의 라이브러리에서 관리하는 실행 단위**이다. (자바의 옛날 Green Thread, Go의 Gorutine)

유저 공간에서만 생성되고 관리되는 논리적인 스레드이며 커널은 유저 레벨 스레드를 인식하지 못한다.

다만 CPU 실행은 커널 레벨 스레드만 할 수 있기 때문에 커널 레벨 스레드 위에서 동작한다.

커널 레벨 스레드가 CPU를 할당받으면, 그 커널 레벨 스레드가 유저 모드로 전환되어 유저 코드를 실행한다.
...
'유저 레벨 스레드가 커널 레벨 스레드 위에서 동작한다'는 유저 코드의 실행 권한과 CPU 접근 권한은 항상 커널 레벨 스레드를 통해서만 가능하다는 것을 의미한다.

컨텍스트 스위칭을 할 때 유저 레벨에서만 이뤄지므로 속도가 매우 빠르다.

유저 레벨 스레드는 커널 스택을 할당받지 않고 스레드가 함수/메서드를 호출하며 사용하는 유저 스택 메모리 공간만 할당받는다.

또한 스레드 라이브러리(`pthread`)가 관리하는 스레드 정보(ID, 상태 등), PC, 레지스터 값 등만을 저장하여 수십 바이트에서 수백 바이트 메모리 공간만 차지한다.

단일 커널 레벨 스레드 위에서의 멀티 플렉싱
- 여러 유저 레벨 스레드가 소수의 커널 레벨 스레드 위에서 작동하기 때문에 많은 수의 유저 스레드가 동시에 존재하더라도 실제로는 단일 커널 레벨 스레드에 의해 실행되므로 메모리 부담이 훨씬 적다.

## 스레드 매핑 모델

커널이 커널 레벨 스레드를 CPU 스케줄링하여 유저 프로세스의 코드를 실행하는 메커니즘은 스레드 매핑 모델을 통해 구현된다.

어떤 모델이든지 결국 유저 레벨 스레드는 커널 레벨 스레드 위에서 동작하게 된다.

CPU가 할당된 커널 레벨 스레드를 실행하는 순간, 이와 연결된 유저 프로세스 내 유저 스레드의 코드와 유저 스택을 바라보며 명령어를 실행하게 된다.

스레드 매핑 모델은 유저 레벨 스레드와 커널 레벨 스레드를 연결하는 방식으로 유저 레벨의 실행 흐름이 커널 스케줄링 단위와 어떻게 매핑되는지를 정의한다.

매핑 모델에 따라 프로세스의 성능, 확장성, 블로킹 동작, 스레드 컨텍스트 스위칭 비용이 달라진다.

스레드 매핑 모델: 1:1, N:1, M:N

#### N:1 (Many-to-One)

N:1 모델은 여러 개의 유저 레벨 스레드가 단일 커널 레벨 스레드에 의해 실행된다
...
유저 공간의 스레드 라이브러리가 스케줄링하므로 시스템 콜을 호출하여 커널 모드로 진입하지 않기 때문에 스레드 간 컨텍스트 스위칭이 매우 빠르다.

운영체제의 스케줄러는 커널이 알고 있는 실행 단위만 CPU에 올릴 수 있는데 N:1 모델에서는 단일 커널 레벨 스레드만 존재하므로, 커널 입장에서 보면 프로세스 내의 실행 흐름이 하나일 뿐이다.

따라서 스레드가 시스템 호출을 하여 블로킹되면 전체 유저 스레드가 블로킹된다.

유저 레벨 스레드가 아무리 많더라도 결국 커널 레벨 스레드는 한 개밖에 되지 못하므로 멀티 코어를 활용할 수 없다.
...
1:1 모델은 하나의 유저 레벨 스레드마다 커널 레벨 스레드를 매핑시킨다.
...
유저 프로그램에서 시스템 콜을 호출하여 스레드 생성을 요청하면 커널은 커널 영역에 커널 레벨 스레드를 만든다. 

신규 커널 레벨 스레드는 유저 영역에 새로 생긴 스레드와 매핑된다.
...
───┼─────────────────┼───┼──────────────┼─── (유저/커널 경계선)
   ▼                 ▼   ▼              ▼
┌─────────────────────────────────────────┐
│  커널 레벨 스레드 1      커널 레벨 스레드 2     │
│  (TCB 1 / 커널스택 1)   (TCB 2 / 커널스택 2)│
│                                         │
│  [ OS 스케줄러 ] ──> CPU 코어에 할당         │
[ 커널 공간 (Kernel Space) ]                │
└─────────────────────────────────────────┘
...
1:1 모델에서 유저 레벨 스레드는 커널 레벨 스레드에 대한 래퍼(wrapper) 개념으로 같은 실행 흐름을 유저 레벨 계층에서 표현한 것 뿐이다.

자바 `Thread` 객체 (유저 공간) -> OS 커널 스레드 (커널 공간)

커널이 모든 스레드를 직접 스케줄링하므로 각 스레드는 독립적으로 CPU에 할당될 수 있다.

멀티코어를 완전히 활용할 수 있으며 하나의 스레드가 블로킹되어도 다른 스레드는 실행을 이어갈 수 있다.

단, 스레드가 증가하게 되므로 커널 부담 증가, 스택 메모리 소비, 컨텍스트 스위칭 비용이 증가한다
...
M:N 모델은 여러 유저 레벨 스레드가 여러 커널 레벨 스레드와 매핑된다.
...
유저 스케줄러와 커널 스케줄러가 협력하여 동작하며, 일부 유저 스레드만이 실제 커널 스레드 위에서 동작한다.

멀티 코어 활용, 커널 레벨 스레드 수를 제한할 수 있다.

높은 확장성과 균형적으로 자원을 사용할 수 있다.

다만 구현이 복잡하고 디버깅이 어렵다는 단점이 있다.
...
**스케줄링**이란 프로세스나 스레드의 실행 순서를 결정하는 작업으로 운영체제가 언제, 어떤 프로세스/스레드를 CPU에 할당하여 실행할지 결정하는 과정을 말한다.

커널의 스케줄링은 보통 실행 중인 프로세스/스레드를 강제로 중단시키고 다른 프로세스/스레드를 실행하는 **선점형(Preemptive) 스케줄링**을 사용한다.

반대로 커널이 프로세스/스레드가 스스로 자원을 반납할 때까지 기다리는 **협력형(Cooperative, 비선점형) 스케줄링**이라고 한다.

프로세스나 스레드가 명령을 수행할 때 필요한 모든 상태를 **컨텍스트**라고 하는데 다음과 같은 요소들이 포함된다.
- 프로그램 카운터(PC): 다음에 실행할 명령어 주소를 보관하는 레지스터
- 레지스터에 보관된 값: 작업 중인 계산, 변수 등을 저장하는 CPU 레지스터
...
스케줄링을 하면 기존 프로세스/스레드의 컨텍스트를 메모리에 저장하고, 새로운 프로세스/스레드 컨텍스트를 CPU 레지스터에 로드하는 작업을 거치게 된다.

이 작업을 **컨텍스트 스위칭(Context Switching)** 이라고 하는데 보통 타임 슬라이스(OS가 할당한 시간) 종료, 블로킹 I/O, 높은 우선순위의 작업, 자발적 yield 등으로 인해 발생한다.

프로세스는 PCB(Process Control Block)라고 하는 프로세스 컨텍스트를 가지는데 커널의 메모리 영역에 존재하여 유저 공간에서 이를 접근할 수 없다.

프로세스 컨텍스트 스위칭이 발생하면 운영체제 메모리 공간에 PCB를 저장하고 기존 저장되어 있던 PCB 또는 새롭게 생성된 PCB를 CPU에 로딩한다.

프로세스 컨텍스트는 가상 주소 공간과 TLB (Transalation Lookaside Buffer, 가상 주소를 물리 주소로 변환하는 캐시)라는 캐시도 포함하고 있는데, 컨텍스트 스위칭 시 새 프로세스는 다른 가상 주소 공간을 사용하므로 이전 TLB 엔트리가 더 이상 유효하지 않게 되어 비우는 작업을 하는데 이를 캐시 무효화(TLB 캐시)라고 한다. 
...
**커널 스레드 컨텍스트**는 **TCB (Thread Control Block)** 에 SP, PC, 레지스터 세트, 상태 레지스터(플래그), TLS(Thread Local Storage) 정도만 보관한다.

프로세스 컨텍스트와 달리 페이지 테이블이나 TLB를 관리하지 않기 때문에 프로세스 컨텍스트 스위칭보다 빠르게 전환되며 캐시 무효화가 발생하지 않게 된다.

**유저 스레드 컨텍스트**는 PC, SP, 레지스터 세트 정도만 가지며 커널 스레드와 달리 커널이 이 존재를 모른다.

또한 TCB도 아닌 유저 공간의 메모리(Heap 등)에 직접 저장되고 커널 호출(시스템 콜)없이 전환될 수 있기 때문에 컨텍스트 스위칭 속도가 가장 빠르다.


## 멀티 코어와 멀티 스레딩 (동시성 문제)

![single, multi core and threads](./assets/single_multi_core_and_threads.png)

[이미지 출처](https://dev.to/a_greynoon/single-and-multi-threaded-programming-with-concurrency-and-parallelism-explain-the-differences-and-concepts-3eec)

하나의 프로세스 안에서 여러 실행 흐름(스레드)이 동시에 실행되는 것을 멀티 스레딩이라고 한다.

멀티 스레딩을 이용하면 응답성을 향상시키거나 자원 효율성과 성능을 높일 수 있다.

운영체제의 스케줄러는 각 커널 스레드에게 CPU를 할당하고 컨텍스트 스위칭하여 멀티스레딩을 구현한다.

싱글 코어 CPU에서는 **동시성(Concurrency)** 을, 멀티 코어 CPU에서는 **병렬성(Parallelism)** 을 구현한다.

![concurrency vs parallelism](./assets/concurrency_parallelism.png)

동시성: 싱글 코어에 작업을 순차적으로 실행 (여러 작업을 완전히 같은 시간에 동시다발적으로 실행할 수 없으나 OS가 빠르게 스위칭하여 여러 작업이 겹치는 시간대에 동시적으로 진행될 수 있도록 하는 개념)

병렬성: 멀티 작업을 병렬적으로 실행 (같은 시간에 동시다발적으로 2개 이상의 작업을 실행하는 개념)

스레드는 프로세스의 자원을 공유하면서 자신만 접근할 수 있는 자원을 가진다.

**공유 자원**: 코드, 데이터(Static/Global 변수), 힙 메모리, 파일 디스크립터, 프로세스 환경 등
...
여러 스레드가 공유 자원에 접근하여 발생하는 문제를 동시성 문제라고 하며, 공유 자원에 접근하는 코드 블록을 임계 구역이라고 한다.

동시성 문제: **경쟁 상태(Race Condition)**, **교착 상태(Deadlock)**, **기아(Starvation)**, **라이브락(Livelock)**
...
`count++`와 같은 코드 한 줄짜리 연산은 컴퓨터 내부적으로 여러 단계의 기계어 명령으로 나뉘어 실행된다.
...
상호 배제란 데이터의 일관성을 유지하기 위해 여러 스레드가 공유 자원에 동시에 접근하지 못하도록 막는 것을 말한다.
...
**동기화를 통해 단 하나의 스레드만이 임계 구역의 코드를 실행하게 함으로써 원자적 연산을 보장한다.**
...
5. 대기 스레드 실행: 반납한 잠금은 대기하던 다른 스레드 중 하나에게 주어지고 해당 스레드가 임계 구역에 진입하여 코드를 실행한다
```
...
그리고 함수를 호출하는 코드 또는 함수를 '호출자(Caller)'라고 하고 호출된 함수를 호출 대상 함수 또는 '피호출자(Callee)'라고 한다.
...
**서브 루틴**은 메인 루틴이나 다른 서브 루틴에 의해 호출되는 함수(메서드)로, 호출자에 의해 호출되어 제어권을 받고 콜 스택에 쌓인 뒤 작업을 수행하고 결과를 반환한 후 스택 프레임을 정리하고 다시 호출자에게 제어권을 넘긴다.
...
- **실행 컨텍스트와 상태**: 서브 루틴이 종료되는 순간 **콜 스택에서 해당 서브 루틴의 스택 프레임이 제거되면서 실행 컨텍스트가 완전히 소멸**된다. 다음에 동일한 서브 루틴이 호출되면 모든 것이 초기화된 상태로 새로 시작한다 (이전 상태를 유지하지 못함)
...
**코루틴**은 'Cooperative Routine'의 약자로 서브루틴의 종속적이고 동기적인 특징을 제거하고 유연한 실행 흐름을 가진다.
...
이 때 **현재 실행 컨텍스트(지역 변수, 코드 실행 위치 등)를 제거하지 않고 그대로 보존(주로 힙 영역에 상태 저장)**한다. (`return`과 달리 제어권을 넘기지만 실행 컨텍스트를 유지함)

호출자는 `yield`를 통해 제어권을 돌려받은 후 자신의 코드를 계속 실행하다가 원하는 시점에 다시 코루틴을 호출(`resume` 또는 `next`)하여 **제어권을 코루틴으로 넘기고 보존된 실행 컨텍스트를 기반으로 중단된 지점부터 실행을 재개**할 수 있다.
...
## 제어권(블로킹과 논블로킹), 작업 순서 흐름(동기와 비동기)

제어권이란 코드를 실행할 수 있는 자격을 의미한다.

제어권을 주고 있는 주체가 코드를 실행할 수 있으며 다른 함수를 호출하면 이 권리가 넘어가게 된다.

제어권이 넘어가면 지금 하던 작업이 멈추게 되고 제어권이 넘어간 코드 블록이 실행된다.

이후 제어권을 넘긴 곳으로부터 제어권을 돌려받아 멈췄던 부분부터 코드를 이어 실행한다.

**블로킹**은 제어권을 통째로 넘겨버리는 방식이다.

호출자가 피호출자를 호출하면서 제어권을 넘기면 호출자는 아무런 코드도 실행할 수 없고 피호출자의 실행이 끝나길 기다리는 대기(Blocked) 상태가 된다.

피호출자가 자신의 실행을 모두 마치고 리턴하면서 제어권을 다시 호출자에게 돌려주면 제어권을 돌려받은 호출자는 멈췄던 부분부터 코드를 이어서 실행한다.

네트워크 소켓 통신에서 `recv()`나 `read()` 시스템 콜을 호출하면 상대방이 데이터를 보낼 때까지 프로그램의 실행을 멈추고 기다리는 현상이 대표적인 블로킹이다.

**논블로킹**은 함수를 호출하더라도 피호출자가 제어권을 붙잡지 않고 곧바로 호출자에게 돌려주는 방식이다.

피호출자는 자신의 코드 실행이 끝나지 않았어도 제어권을 호출자에게 즉시 반환한다. (피호출자는 실행을 접수한 느낌)

제어권을 돌려받은 호출자는 다음 코드를 계속 실행할 수 있다.

블로킹과 논블로킹이 제어권과 관련된 개념이라면 동기와 비동기는 '작업의 완료 시점을 맞추느냐'와 관련된 개념이다.

**동기**는 피호출자의 실행 종료 시점과 호출자의 다음 작업 시작 시점을 맞추는 방식이다.

즉, 순서대로 하나씩 처리하는 것을 의미한다.

**비동기**는 피호출자의 실행 종료 시점과 호출자의 다음 작업 시작 시점을 맞추지 않는 방식이다.

호출자는 콜백 함수나 이벤트 등을 통해 피호출자 실행 결과를 전달받는다.

일반적으로 동기/블로킹과 비동기/논블로킹 조합의 프로그래밍 모델을 사용한다.
